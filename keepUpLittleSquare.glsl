...
float dtoa(float d, float amount) { return 1. / clamp(d * amount, 1., amount); }
float sdSquare(vec2 p, vec2 pos, vec2 origin, float a, float s) {
  vec2 d =
      abs(((p - pos) * mat2(cos(a), sin(a), -sin(a), cos(a))) - origin) - s;
...
void scene(out vec4 o, vec2 N, vec2 uv, float t, float xpos, float xcam) {
  ...
  float bigHeightThis =
      mix(bigCellThis.y, bigCellNext.y, bigCellThis.z); // terrain height
  ...
  float aNext =
      atan(sqCellThis.y - sqCellNext.y, sqCellThis.x - sqCellNext.x) - PI * .5;
  if (aNext > aThis + PI)
    aNext -= PI2;
  if (aNext < aThis - PI)
    aNext += PI2;
  ...
  sdsq = sdSquare(uv, vec2(sqCellThis.x, sqCellThis.y), vec2(-sz * .5, sz * .5),
                  asq + PI, sz * .5);
...
void mainImage(out vec4 o, vec2 fragCoord) {
  ...
  vec3 sqCellPrev = cell(xpos - xscale);
  float bounce =
      abs(sin(sqCellPrev.z * 26.)) * pow(1. - sqCellPrev.z, 7.) * .03;
  uv.y += bounce;
